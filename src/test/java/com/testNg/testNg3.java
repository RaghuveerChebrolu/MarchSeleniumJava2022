...
public class testNg3 extends library_CommonBusinessFunctions{
	@Test(priority = 1)
	public void ValidateGmoOnlinesuccessfully() {
		System.out.println("inside testCase1");
		String title = driver.getTitle();
		System.out.println("title:" + title);
		Assert.assertEquals(title, objProp.getProperty("GMO_onlineTitle"));
	}

	@Test(priority = 2,dependsOnMethods= {"ValidateGmoOnlinesuccessfully"})
	public void EnterGmoOnline() {
		System.out.println("inside EnterGmoOnline");
		driver.findElement(By.name("bSubmit")).click();
		driver.findElement(By.xpath("//input[@name='QTY_BACKPACKS']")).sendKeys(objProp.getProperty("QTY_BACKPACKS"));
		driver.findElement(By.name("bSubmit")).click();
		String title = driver.findElement(By.xpath("//h1[contains(text(),'Place')]")).getText();
		System.out.println("title:" + title);
		Assert.assertEquals(title, objProp.getProperty("GMOonlinePlaceOrderTitle"));

	}

	@Test(priority = 3,dependsOnMethods= {"ValidateGmoOnlinesuccessfully","EnterGmoOnline"})
